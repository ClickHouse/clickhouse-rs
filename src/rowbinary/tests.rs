...
fn it_deserializes() {
    ...
    for i in 0..input.len() {
        ...
        // It shouldn't panic.
        let _: Result<Sample<'_>, _> = super::deserialize_from(&mut left, &[]).0;
        let _: Result<Sample<'_>, _> = super::deserialize_from(&mut right, &[]).0;

        let actual: Sample<'_> = super::deserialize_from(&mut input.as_slice(), &[])
            .0
            .unwrap();
