...
struct TimeExampleChrono {
    ...
    #[serde(with = "clickhouse_rs::serde::chrono::time64::secs")]
    time64_seconds: chrono::NaiveTime,
    
    #[serde(with = "clickhouse_rs::serde::chrono::time64::millis")]
    time64_millis: chrono::NaiveTime,
    
    #[serde(with = "clickhouse_rs::serde::chrono::time64::micros")]
    time64_micros: chrono::NaiveTime,
    
    #[serde(with = "clickhouse_rs::serde::chrono::time64::nanos")]
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client.insert("time_example", vec![time_example_chrono]).await?;
    
    // Query the data
    let rows = client.query("SELECT * FROM time_example ORDER BY time_field").await?;
